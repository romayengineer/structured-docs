...
func (c *Compiler) Compile(cfg *config.Config) ([]Result, error) {
	...
	if cfg.GenerateReadme == nil || *cfg.GenerateReadme {
		readmeName := cfg.ReadmePath
		if readmeName == "" {
			readmeName = "README.md"
		}
		readmePath := filepath.Join(cfg.OutputDir, readmeName)
		...
		for _, r := range sorted {
			name := strings.TrimSuffix(r.SourcePath, ".yml")
			name = strings.TrimSuffix(name, ".yaml")
			readmeDir, _ := filepath.Abs(filepath.Dir(readmePath))
			outAbs, _ := filepath.Abs(r.OutputPath)
			rel, _ := filepath.Rel(readmeDir, outAbs)
			fmt.Fprintf(&b, "- [%s](%s)\n", name, rel)
		}
		b.WriteString("\n---\n\nGenerated by **sd**\n")

		if err := c.FS.MkdirAll(filepath.Dir(readmePath), 0755); err != nil {
			return nil, fmt.Errorf("creating directory for README: %w", err)
		}
		if err := c.FS.WriteFile(readmePath, []byte(b.String()), 0644); err != nil {
