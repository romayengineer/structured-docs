...
func TestIntegration_ReadmeCreated(t *testing.T) {
	mem := fsys.NewMemFS()
	writeProject(mem, "post-a.yml", "content a")
	writeProject(mem, "post-b.yml", "content b")
	cfg := &config.Config{
		SchemaDir: "schema", DataDir: "data", TemplateDir: "templates",
		OutputDir: "output", TemplateOrder: []string{"post.template.md"},
	}
	...
	b, err := mem.ReadFile("output/README.md")
	if err != nil {
		t.Fatal("README.md not found")
	}
	content := string(b)
	if !strings.Contains(content, "post-a") {
		t.Error("README should contain post-a")
	}
	if !strings.Contains(content, "post-b") {
		t.Error("README should contain post-b")
	}
	if !strings.Contains(content, "Generated by **sd**") {
		t.Error("README should contain generator footer")
	}
}

func TestIntegration_ReadmeDisabled(t *testing.T) {
	mem := fsys.NewMemFS()
	...
	f := false
	cfg.GenerateReadme = &f
	_, err := compiler.Compile(mem, cfg)
	if err != nil {
		t.Fatalf("expected success, got: %v", err)
	}
	_, err = mem.ReadFile("output/README.md")
	if err == nil {
		t.Fatal("README.md should not exist when generate_readme=false")
	}
}
...
func TestIntegration_ReadmeDefaultEnabled(t *testing.T) {
	...
	_, err = mem.ReadFile("output/README.md")
	if err != nil {
		t.Fatal("README.md should exist by default")
	}
}
